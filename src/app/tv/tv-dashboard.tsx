...
export function TvDashboard() {
  const { project, zones, loadWorkspace } = useProjectStore();
  useEffect(() => { void loadWorkspace(); }, [loadWorkspace]);
  return <main className="min-h-screen bg-obsidian p-8 text-white">
    <div className="mb-6 flex items-center justify-between"><div><p className="text-sm uppercase tracking-[0.35em] text-muted">War room / kiosk</p><h1 className="text-4xl font-black">{project?.name ?? 'Sin proyecto'}</h1></div><div className="text-right text-muted">Modo presentación<br />TV 4K listo</div></div>
