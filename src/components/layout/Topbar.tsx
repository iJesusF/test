...
export function Topbar() {
  ...
  return <header className="flex flex-wrap items-center gap-3 border-b border-white/10 bg-graphite/80 px-4 py-3 backdrop-blur-xl lg:px-6">
    ...
    <label aria-disabled={isUploading} className={`relative inline-flex items-center gap-2 overflow-hidden rounded-xl bg-electric px-3 py-2 text-sm font-semibold text-white shadow-glow transition hover:bg-blue-400 ${isUploading ? 'cursor-wait opacity-60' : 'cursor-pointer'}`}>
      <input type="file" accept="image/png,image/jpeg,image/webp,application/pdf" onChange={handleUpload} className="absolute inset-0 h-full w-full cursor-pointer opacity-0" disabled={isUploading} aria-label="Subir plano" />
