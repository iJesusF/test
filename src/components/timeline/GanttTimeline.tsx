...
export function GanttTimeline() {
  ...
  const start = sortedDates[0] ? addDays(sortedDates[0], -2) : new Date();
  const end = sortedDates.at(-1) ?? addDays(start, 14);
  const dayCount = Math.max(14, differenceInCalendarDays(end, start) + 5);
  const days = Array.from({ length: dayCount }, (_, index) => addDays(start, index));
  const height = Math.max(tasks.length * rowHeight + 44, 140);
  return <section className="rounded-[2rem] border border-white/10 bg-graphite/80 p-4 shadow-panel backdrop-blur">
    <div className="mb-4 flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.28em] text-muted">Cronograma</p><h2 className="text-lg font-semibold text-white">Gantt generado desde zonas reales</h2></div><div className="rounded-xl border border-white/10 px-3 py-2 text-sm text-muted">Semana / Mes</div></div>
    {!tasks.length ? <div className="rounded-2xl border border-dashed border-white/10 p-6 text-sm text-muted">Dibuja una zona en el plano para crear automáticamente una barra de tarea editable desde el panel lateral.</div> : <div className="overflow-x-auto"><div className="grid min-w-[1060px] grid-cols-[220px_1fr]">
