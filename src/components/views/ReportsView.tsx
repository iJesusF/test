...
export function ReportsView() {
  ...
  return <section className="rounded-[2rem] border border-white/10 bg-graphite/80 p-4 shadow-panel backdrop-blur">
    <p className="text-xs uppercase tracking-[0.28em] text-muted">Reportes</p><h2 className="mb-4 text-lg font-semibold text-white">Reporte Supabase del avance</h2>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><MetricCard label="Avance global" value={`${project?.progress ?? 0}%`} /><MetricCard label="Planos" value={String(floorplans.length)} /><MetricCard label="Completadas" value={`${completed}/${zones.length}`} /><MetricCard label="Bloqueadas" value={String(blocked)} /></div>
